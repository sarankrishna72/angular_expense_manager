...
export class TransactionService {
  ...
  transactionsGroupByMonth = computed(() => {
    if (this.transactions().length > 0) {
      const groupedByMonth = _.groupBy(this.transactions(), (transaction) => {
  ...
  mapLastSixMonthsEmptyData(groupedByMonth) {
  ...
  getLastSixMonthsTransactions() {
    collectionData(this.transactionCollection, {}).subscribe(data => {
      ...
      let transactions =  _.filter( _.map(_.orderBy(data, ['transaction_date'], ['desc']), transaction => new TransactionModel(transaction)), transaction => {
      ...
      // this.transactions.set(JSON.parse(JSON.stringify(transactions)));
    })
  }
